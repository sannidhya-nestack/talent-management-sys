...
export async function getUserGmailCredential(
  ...
  const credential = await db.gmailCredential.findUnique({
    where: { userId },
  });
  ...
  return {
    ...
    refreshToken: decryptToken(credential.refreshToken),
...
export async function getAllGmailAccounts(): Promise<GmailAccountInfo[]> {
  const credentials = await db.gmailCredential.findMany({
    include: {
      user: {
        select: {
          displayName: true,
...
export async function disconnectGmail(userId: string): Promise<void> {
  ...
  if (credential) {
    ...
    await db.gmailCredential.delete({
      where: { userId },
    });
  }
}
...
export async function hasGmailConnected(userId: string): Promise<boolean> {
  const count = await db.gmailCredential.count({
    where: { userId },
  });
  return count > 0;
}
